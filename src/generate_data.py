...
from src.mutils import generate_tsp_data_mg

def generate_tsp_data_uniform(dataset_size, tsp_size):
...
def generate_vrp_data(dataset_size, vrp_size):
    ...
    loc=np.random.uniform(size=(dataset_size, vrp_size, 2)).tolist()  # Node locations
    return list(zip(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generate_type', type=str,required=True, help="mg or uniform")
        ...
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if opts.filename is None:
                    ...
                    size_desc="{}M".format(size//1000000) if size>=1e6 else "{}K".format(size//1000)
                    filename = os.path.join(datadir, "{}{}{}_{}_seed{}_size{}.pkl".format(
                        problem,
                        "_{}".format(distribution) if distribution is not None else "",
                        graph_size, opts.name, opts.seed,size_desc))
                else:
                ...
                if problem == 'tsp':
                    if opts.generate_type=='mg':
                        dataset = generate_tsp_data_mg(opts.dataset_size)
                    else:
                        dataset = generate_tsp_data_uniform(opts.dataset_size, graph_size)
                elif problem == 'vrp':
